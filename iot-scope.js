module.exports = function (RED) {
		...
		node.on('input', function (msg) {
			if (mqttClient.connected) {
				...
				if (timeDiff < 900) {
					...
					//console.log("Sending IoT Scope Data");
					startTime = new Date();
				}
			}
		});
	...
	RED.nodes.registerType("iot-scope", IoTScope);
	
	function IoTSql(config) {
		RED.nodes.createNode(this, config);
		...
		var publishTopic = "CBUS/MQTT/SQL/" + node.iotId;
		var startTime, endTime;
		...
		node.on('input', function (msg) {
			if (mqttClient.connected) {
				...
				if (timeDiff < 59500) {
					console.log("trigger interval is 60 seconds!");
					return;
				}
				else {
					mqttClient.publish(publishTopic, JSON.stringify(json));
					//console.log("Sending IoT SQL Data");
					startTime = new Date();
				}
			}
		});
	...
	RED.nodes.registerType("iot-sql", IoTSql);

	function IoTAe(config) {
		RED.nodes.createNode(this, config);
		...
		var publishTopic = "CBUS/MQTT/AE/" + node.iotId + "/Normal";
		var startTime, endTime;
		...
		node.on('input', function (msg) {
			if (mqttClient.connected) {
				...
				var json = { "Date": createTimestamp(), "Message" : msg.payload, "Value":"0", "Unit":"" };
				...
				if (timeDiff < 900) {
					console.log("trigger interval is 1 seconds!");
					return;
				}
				else {
					mqttClient.publish(publishTopic, JSON.stringify(json));
	...
	RED.nodes.registerType("iot-ae", IoTAe);
}